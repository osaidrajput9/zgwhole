 ...
 * piece of markup gets, per the rule:
 *
...
function wire() {
  document
    .querySelectorAll("[data-reveal-h1]")
  ...
  /* lift() is a gsap.from, so it animates towards whatever opacity the
     element currently has. The CSS above starts it at 0 to avoid a flash,
     which would otherwise make the tween run 0 → 0 and never appear. The
     end state is set first; both calls land in the same frame, so the
     from-tween's immediate render puts it straight back to hidden without
     anything being painted in between. */
  const liftIn = (items, trigger) => {
    if (!items.length) return;
    gsap.set(items, { opacity: 1 });
    lift(items, { trigger });
  };

  /* Grouped content staggers against its own container, so a section
     low on the page does not fire off a trigger at the top of it. */
  document.querySelectorAll("[data-lift-group]").forEach((group) => {
    liftIn(group.querySelectorAll("[data-lift]"), group);
  });
